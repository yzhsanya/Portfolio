...
class GameTests: XCTestCase {
    ...
    func testInstanceValueTypesPermitted(){
        for child in mirror.children {
            let isString = type(of: child.value) == type(of: String())
            let isInt = type(of: child.value) == type(of: Int())
            let isPublishedString = type(of: child.value) == type(of: Published<String>(wrappedValue: ""))
            let isPublishedInt = type(of: child.value) == type(of: Published<Int>(wrappedValue: 0))

            XCTAssertTrue(isString || isInt || isPublishedString || isPublishedInt , "The only allowable types for instance variables in the Game class are String and Int")
        }
    }
    
    
    func testNotASubclass(){
        XCTAssertNil(mirror.superclassMirror, "You are not permitted to make the Game class as subclass of another class")
    }
...
class GamePointsTests : XCTestCase{
